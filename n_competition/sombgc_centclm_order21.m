function dxdt=sombgc_centclm_order21(t,x,Extra)
%
%
...
xt=dxdt(vid.minn)*Extra.dt+x(vid.minn);
if(xt<0)
    dxdt(vid.minn)=dxdt(vid.minn)-xt/Extra.dt;
end

dxdt(vid.minp)=sum(rdp)-par.lchp*x(vid.minp);
xt=dxdt(vid.minp)*Extra.dt+x(vid.minp);
if(xt<0)
    dxdt(vid.minp)=dxdt(vid.minp)-xt/Extra.dt;
end
