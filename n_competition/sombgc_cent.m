function dxdt=sombgc_cent(t,x,Extra)
%
%
...
dxdt(vid.minn)=sum(rdn);                              %total n demand, including closed coupling between mobilizer and immobilizer
dxdt(vid.minp)=sum(rdp);                              %total p demand, including closed coupling between mobilizer and immobilizer

if(-dxdt(vid.minn)*Extra.dt>x(vid.minn))
    alpha_n=x(vid.minn)/(-dxdt(vid.minn)*Extra.dt);
else
    alpha_n=1;
end

if(-dxdt(vid.minp)*Extra.dt>x(vid.minp))
    alpha_p=x(vid.minp)/(-dxdt(vid.minp)*Extra.dt);
else
    alpha_p=1;
end
