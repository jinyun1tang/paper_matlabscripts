function dxdt=sombgc_centeca(t,x,Extra)
%
%
...
rdp(vid.som2)=(1./par.rcp(vid.som2)-par.som2_to_som1/par.rcp(vid.som1)-par.som2_to_som3/par.rcn(vid.som3))*rdc(vid.som2);
...
%compute the eca denominator for nitrogen
eca_denorm_n=par.k_minn*(1+par.lchn);
eca_denorm_p=par.k_minp*(1+par.lchp);
for j = 1 : length(vid.ids)
    if(rdn(vid.ids(j))<0)
        eca_denorm_n=eca_denorm_n+x(vid.ids(j))*par.pct_mcb;
    end
    if(rdp(vid.ids(j))<0)
        eca_denorm_p=eca_denorm_p+x(vid.ids(j))*par.pct_mcb;
    end
end
%compute the eca factor for phosphorus
...
for j = 1 : length(vid.ids)
    ...
    alpha_p=1;
    if(rdn(vid.ids(j))<0)
        alpha_n=x(vid.minn)/eca_denorm_n;        
    end
    if(rdp(vid.ids(j))<0)
        alpha_p=x(vid.minp)/eca_denorm_p;
    end
...
dxdt(vid.minn)=sum(rdn)-par.lchn*x(vid.minn);                              %total n demand, including closed coupling between mobilizer and immobilizer
dxdt(vid.minp)=sum(rdp)-par.lchp*x(vid.minn);                              %total p demand, including closed coupling between mobilizer and immobilizer
