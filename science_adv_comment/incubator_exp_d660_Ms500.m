...
ofile=[mfolder,'/Incubator_Ms500_days660.mat'];
load(ofile);
Extra.id_f1=1;
Extra.id_k1_tl=2;
Extra.id_k2_tl=3;
...
Extra.t=(1:incubator.days);

Extra.f1_0=5.76e-2;
Extra.k1_0=8.68e-3;
Extra.k2_0=0.74e-4;
%generate synthetic measurements
Extra.obs_csh_tl=incubator.cumresp_T10C(2:end).*(ones(size(Extra.t))+...
    0.02.*randn(size(Extra.t)));

Extra.obs_csh_th=incubator.cumresp_T20C(2:end).*(ones(size(Extra.t))+...
    0.02.*randn(size(Extra.t)));
...
set_curAX(fig,ax(1));
histogram(Z(end-eld:end,1),50);title('f1','FontSize',14);
mn=mean(Z(end-eld:end,1));sgm=std(Z(end-eld:end,1));
tag=['f_1=',num2str(mn.*Extra.f1_0,'%.2f'),'\pm',num2str(sgm.*Extra.f1_0,'%.2f')];
put_tag(fig,ax(1),[0.5,0.8],tag,fontsz);
xf(1)=mn;

set_curAX(fig,ax(2));
histogram(Z(end-eld:end,2),50);title('k_1 (day ^-^1)','FontSize',14);
mn=mean(Z(end-eld:end,2));sgm=std(Z(end-eld:end,2));
tag=['k_1=',num2str(mn*Extra.k1_0,'%.2e'),'\pm',num2str(sgm*Extra.f1_0,'%.2e')];
put_tag(fig,ax(2),[0.5,0.8],tag,fontsz);
xf(2)=mn;

set_curAX(fig,ax(3));
histogram(Z(end-eld:end,3),50);title('k_2  (day ^-^1)','FontSize',14);
mn=mean(Z(end-eld:end,3));sgm=std(Z(end-eld:end,3));
...
set_curAX(fig,ax(4));
histogram(Z(end-eld:end,4),50);title('k_2  (day ^-^1)','FontSize',14);
mn=mean(Z(end-eld:end,4));sgm=std(Z(end-eld:end,4));
tag=['k_1=',num2str(mn*Extra.k1_0,'%.2e'),'\pm',num2str(sgm*Extra.k1_0,'%.2e')];
put_tag(fig,ax(4),[0.5,0.8],tag,fontsz);
xf(2)=mn;

set_curAX(fig,ax(5));
histogram(Z(end-eld:end,5),50);title('k_2  (day ^-^1)','FontSize',14);
mn=mean(Z(end-eld:end,5));sgm=std(Z(end-eld:end,5));
tag=['k_2=',num2str(mn*Extra.k2_0,'%.2e'),'\pm',num2str(sgm*Extra.k2_0,'%.2e')];
...
set_curAX(fig,ax(6));
plot(Extra.t,cumresp_tl,'b.');
hold on;
plot(Extra.t,Extra.obs_csh_tl,'r.');

plot(Extra.t,cumresp_th,'b.');

plot(Extra.t,Extra.obs_csh_th,'r.');

h=legend('Two-pool fitting','ReSOM output');
xlabel('Days','FontSize',fontsz);
ylabel('Cumulative respiration (g C)','FontSize',fontsz);
