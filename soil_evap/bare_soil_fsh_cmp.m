...
for j = 1 : 4
    ...
    qsoi_ch_beta=get_clm_data(ncf3, var, id_season(:,j),wt(id_season(:,j)),scr, dshft);
    ...
    [c,h]=m_contourf(lon,lat,df,(-3:0.5:3));set(h,'Color','none');
    if(j==1)
...
set(hc,'FontSize',14);
put_tag(fig,ax(3),[1.05,1.95],'\DeltaH_s_o_i',12);
put_tag(fig,ax(3),[1.01,1.85],'(W m^-^2)',12);
...
m_proj('miller','lat',82);
m_coast('color','k');hold on;
[c,h]=m_contourf(lon,lat,df1);set(h,'Color','none');colorbar;
m_grid('linestyle','none','box','fancy','tickdir','out','FontSize',12);put_tag(fig2,ax,[1.04,1.1],'\DeltaH_s_o_i (W m^-^2)',12);
