...
etdif_hdr_yr=(etdif_hdr_djf+etdif_hdr_mam+etdif_hdr_jja+etdif_hdr_son)./4;
%reset
id=find(etdif_hdr_djf>1.5);etdif_hdr_djf(id)=1.5;
id=find(etdif_hdr_djf<-1.5);etdif_hdr_djf(id)=-1.5;
%
id=find(etdif_hdr_mam>1.5);etdif_hdr_mam(id)=1.5;
id=find(etdif_hdr_mam<-1.5);etdif_hdr_mam(id)=-1.5;
%
id=find(etdif_hdr_jja>1.5);etdif_hdr_jja(id)=1.5;
id=find(etdif_hdr_jja<-1.5);etdif_hdr_jja(id)=-1.5;
%
id=find(etdif_hdr_son>1.5);etdif_hdr_son(id)=1.5;
id=find(etdif_hdr_son<-1.5);etdif_hdr_son(id)=-1.5;
...
m_proj('miller','lat',82);
m_coast('color','k');hold on;
[c,h]=m_contourf(lon,lat,etdif_hdr_yr');set(h,'Color','none');colorbar;
m_grid('linestyle','none','box','fancy','tickdir','out','FontSize',12);
