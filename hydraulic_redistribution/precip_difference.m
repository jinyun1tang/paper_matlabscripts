...
j=1;
ncf1=[mdir,ref_files{j},'/','RAIN','.',ref_files{j},'.51-60.nc'];
ncf2=[mdir,ref_files{j},'/','SNOW','.',ref_files{j},'.51-60.nc'];

ncf_map=[mdir,'area_info.nc'];
qsoil=netcdf(ncf1,'var','RAIN');
qvegt=netcdf(ncf2,'var','SNOW');
id=find(abs(qsoil)>1d3);
qsoil(id)=0./0.;
qet_oxi=(qsoil+qvegt).*86400;
...
set(hc,'FontSize',14);
put_tag(fig,ax(2),[0.4,1.02],'CRU-QIAN \DeltaPREC (mm day^-^1)',fontsz);
