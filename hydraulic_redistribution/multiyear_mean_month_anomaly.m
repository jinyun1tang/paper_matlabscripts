function [lh_month_mean,lh_month_std]=multiyear_mean_month_anomaly(lh_subset)
...
for j1 = 1 : sz1
    for j2 = 1 : sz2
        ...
        if(~any(isnan(dat_reshape)))       
            ...
            %obtain mean for each year
            year_mean=mean(dat_reshape,1);
            %detrend the data
            
            lh_month_mean(j1,j2,:)=mean(dat_reshape-repmat(year_mean,[12,1]),2);
