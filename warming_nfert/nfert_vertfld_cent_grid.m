function [dat,lon,lat] = nfert_vertfld_cent_grid(varname,isweighted)
%varname='NEE';
...
[gpp_n06,lon,lat]=vertfld_grid_extract(varname,ncf7,region);
...
gpp_n06_avg=zeros(sz1,sz2,sz3,sz4)+0./0.;
        ...
        for m = 1 : sz3
            if(~any(isnan(gpp_ctl(j,k,m,:))))
                if(isweighted)
                    ...
                    gpp_n06_avg(j,k,m,:)=slidew_mean(gpp_n06(j,k,m,:),12,w);
                    ...
                    gpp_n06_avg(j,k,m,:)=slide_mean(gpp_n06(j,k,m,:),12);                   
                end
            end
        end
    end
end
dat=zeros(size(gpp_ctl_avg,1),size(gpp_ctl_avg,2),size(gpp_ctl_avg,3),size(gpp_ctl_avg,4),7);
dat(:,:,:,:,1)=gpp_ctl_avg;
dat(:,:,:,:,2)=gpp_n01_avg;
dat(:,:,:,:,3)=gpp_n02_avg;
dat(:,:,:,:,4)=gpp_n03_avg;
