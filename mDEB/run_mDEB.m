...
axs(2)=subplot(3,1,2);
plot(t1,(y(:,par.vid.xC)+1).*y(:,par.vid.mV).*1.e-3);
hold on;
plot(C_conc(:,1),C_conc(:,2),'+');xlim([20,30]);
ylabel('POC concentration (M) $\times 10^{-3}$', 'Interpreter', 'latex');
axs(3)=subplot(3,1,3);
plot(t1,(y(:,par.vid.xN)+par.nV_N).*y(:,par.vid.mV).*1.e-2);hold on;
plot(N_conc(:,1),N_conc(:,2),'+');xlim([20,30]);
...
put_tag(fig,axs(1),[.05,.9],'(a)',16);
put_tag(fig,axs(2),[.05,.9],'(b)',16);
put_tag(fig,axs(3),[.05,.9],'(c)',16);
tight_layout();
